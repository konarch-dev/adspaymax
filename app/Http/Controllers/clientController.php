...
class clientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $storeproduct = 'DB'::table('products')->select(
            'users.store',
            'products.name',
            'products.code',
            'products.id',
            ...
            'users.state',
            'users.city'
        )->where('products.status', 1)->join('users', 'users.id', '=', 'products.user_id')->orderBy('products.id', 'desc')->limit(8)->get();
    ...
    public function couponDetail($id)
    {
        ...
        $storeproduct = 'DB'::table('products')->select(
            ...
            'users.state',
            'users.city'
        )->where('products.status', 1)->where('products.id', $id)->join('users', 'users.id', '=', 'products.user_id')->get();
    ...
    public function store(Request $request)
    {
        ...
        return view('client.store', ['category' => $category, 'menu' => $menu, 'store' => [
            'a' => $a, 'b' => $b, 'c' => $c, 'd' => $d, 'e' => $e,
    ...
    public function categoryList($id)
    {
        ...
        if ($category1->parent_id != 0) {
            $product = Product::select(
                ...
                'users.store',
                'users.id'
            )->join('users', 'users.id', '=', 'products.user_id')->where('category', $category1->id)->get();
        } else {
            ...
            $product = Product::select(
                ...
                'users.store',
                'users.id'
            )->join('users', 'users.id', '=', 'products.user_id')->whereIn('products.category', $tmpid)->get();
        }
    ...
    public function register(Request $request)
    {
        ...
        if ($selUser > 0) {
            ...
            if ($validator->fails()) {
                ...
                'Mail'::to($request->email)->send(new \App\Mail\adminpassword($details));
    ...
    public function scan($id)
    {
        ...
        $storeproduct = 'DB'::table('products')->select(
            ...
            'users.state',
            'users.city'
        )->where('products.status', 1)->where('products.id', $coupondata[0]->product_id)->join('users', 'users.id', '=', 'products.user_id')->get();
